...
xsd_add: str = "./TestDir/sources/Add_Entrant_List.xsd"
xsd_get: str = "./TestDir/sources/Get_Entrant_List_2.xsd"
xsd_add_example: str = "./TestDir/sources/Add_example.xml"
fields_class: str = "./TestDir/sources/dict_document_type_cls.json"
xml_example: str = "./TestDir/sources/Get_example.xml"
...
def xml_tree(json_data: dict) -> Tuple[list, str]:
    ...
    for item in tree.getiterator():
        if (item.tag == "Identification"):
            field_el = ET.SubElement(item,'Fields')
            for tag in new_tags:
